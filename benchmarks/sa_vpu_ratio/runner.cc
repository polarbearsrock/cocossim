...
struct Oracle {
  ...
  template <class S, class J> Timing measure(S &s, J &j) {
    // Native SA init records its dispatch timestamp. Reset BEFORE init so
    // a previous oracle measurement cannot create a negative elapsed time.
    gcycles = 0;
    s.j = &j;
    s.init();
...
struct Layer {
  ...
  Graph build(bool attention_only = false) {
    ...
    U expected = U(tokens)*(U(2)*D*D + U(2)*D*KV*HD)
               + U(2)*batch*H*seq*seq*HD;
    if (!attention_only) expected += U(3)*tokens*D*FF;
    require(g.work()[SA] == expected, "Qwen layer MAC census failed");
    U vector_expected = U(tokens)*(4*D+2+nonlinear)
                      + U(tokens)*(D+2*KV*HD)
                      + U(3)*tokens*(D+KV*HD)
                      + U(batch)*H*seq*((6+nonlinear)*seq+nonlinear)
                      + U(tokens)*D;
    if (!attention_only) vector_expected += U(tokens)*(4*D+2+nonlinear)
                      + U(tokens)*D + U(tokens)*FF*(4+2*nonlinear);
...
struct StageTrace { U ready=UINT64_MAX, first=UINT64_MAX, last=0; };
struct Result {
  U cycles=0;
...
Result simulate(const Graph &g, int nsa, int nvu, U bin_width=100000, bool tick_reference=false) {
  ...
  require(nsa > 0 && nvu > 0 && bin_width > 0, "unit counts and bin width must be positive");
  ...
  Result out;
  out.stages.resize(g.stages.size());
  ...
  while(completed<g.stages.size()) {
    for(int t=0;t<2;++t) while(free[t]>0 && !ready[t].empty()) {
      ...
      out.stages[id].first=std::min(out.stages[id].first,now);
  ...
  for(size_t i=0;i<g.stages.size();++i) {
    const auto &s=out.stages[i];
    require(s.ready<=s.first && s.first<s.last,"invalid stage execution interval");
    for(int dep:g.stages[i].deps)
      require(out.stages[dep].last<=s.ready,"stage ran before its producer completed");
  }
  return out;
}
...
void selftest(Oracle &o) {
  ...
  o.vec(1,16384,12);
  auto short_sa=o.sa(32,128,32);
  require(short_sa.cycles==161,"SA timing depends on previous oracle measurement");
  o.vec(1024,128,2);
  require(o.sa(32,1024,32).cycles==1057,"SA dispatch clock regression");
  ...
  Layer group{o,{},1,1024,1};
  int score=group.gemm("score","QK",128,128,1024,{});
  int sm=group.g.add("softmax","Softmax",VPU,{{1,o.softmax(1024,128,1)}},{score});
  group.gemm("av","AV",128,1024,128,{sm});
  require(simulate(group.g,2,1).cycles==10304+7175+8456,
          "two-SA/one-VPU attention group timing failed");
  Layer attention{o,{},1,17,1}; auto ag=attention.build(true);
  require(ag.stages.back().name=="residual1","attention scope includes extra work");
  simulate(ag,2,1);
  std::mt19937 rng(42);
  ...
  std::cout<<"PASS: native timings and clock isolation, tail tiles, attention scope, "
           <<"two-SA attention group, hand schedule, and 50 event/cycle schedule comparisons\n";
}
...
int main(int argc,char **argv) {
  try {
    ...
    for(int i=1;i<argc;++i) {
      ...
      else if(a=="--scope") scope=value;
      else if(a=="--bin-width") bin_width=std::stoull(value);
      else if(a=="--trace-stages") {require(value=="0" || value=="1","trace-stages must be 0 or 1");trace_stages=value=="1";}
    ...
    require(scope=="full" || scope=="attention","scope must be full or attention");
    require(bin_width>0,"bin width must be positive");
    Oracle oracle;
    ...
    std::ofstream stages(output+"/stages.csv");
    require(summary && timeline && classes && census && stages,"could not open result files");
    ...
    stages<<"nonlinear_cost,vpu_count,stage_id,name,category,resource,ready_cycle,first_cycle,last_cycle,jobs,service_cycles,work,dependencies\n";
    summary<<std::setprecision(12);timeline<<std::setprecision(9);
    for(int nl:nlcosts) {
      Layer model{oracle,{},batch,seq,nl}; Graph graph=model.build(scope=="attention");
      bool first=true;
      for(int nv:vpus) {
        auto r=simulate(graph,nsa,nv,bin_width);
        ...
        for(auto &[bin,b]:r.bins) timeline<<nl<<','<<nv<<','<<bin*bin_width<<','<<b.duration<<','
        ...
        for(size_t id=0;trace_stages && id<graph.stages.size();++id) {
          const auto &s=graph.stages[id]; const auto &t=r.stages[id];
          U service=0,work=0;
          for(const auto &b:s.batches) {service+=b.count*b.timing.cycles;work+=b.count*b.timing.work;}
          stages<<nl<<','<<nv<<','<<id<<','<<s.name<<','<<s.category<<','<<(s.type==SA?"SA":"VPU")
                <<','<<t.ready<<','<<t.first<<','<<t.last<<','<<s.count()<<','<<service<<','<<work<<',';
          for(size_t d=0;d<s.deps.size();++d) {if(d) stages<<';';stages<<s.deps[d];}
          stages<<'\n';
        }
        first=false;
