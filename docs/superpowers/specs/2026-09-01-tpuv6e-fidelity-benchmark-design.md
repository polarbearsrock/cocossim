...
| K1 (paged-KV gather, `tier1/k1_kv_gather.csv`) | Shuffled vs sequential block table: ratio 1.00 in every cell — page placement costs nothing on v6e. The legacy paged kernel is 3.9–7.3× slower than XLA's dense attention over the same KV (dense: 30 µs at 512×8, 203 µs at 2048×32, 1004 µs at 8192×32), and dense XLA runs at RPA speed (2048×32: 203 µs vs 166 µs in-model). Against the dense cost the sim is −28% (512×8), +5% (2048×32), +46% (8192×8), −14% (8192×32): its decode attention is within 1.5× everywhere and the K1-fitted `-kv_bw_pct` (25) is void because it targets the legacy kernel. Caveat: K1 divides chained wall time by chain 8, so ~14 µs of host floor sits in every K1 per-step number; ratios are unaffected, but the ≤ 100 µs cells' absolute times are high by 10–50% (the 30 µs dense figure at 512×8 is ≈ 16 µs device time). A1 prefill `per_step_us` includes the scan-carry copy outside the kernel (≤ 7% at 2048×8, 1% at 8192×1; `kernel_us` in the CSV is the kernel alone). |
