...
| K1 (paged-KV gather, `tier1/k1_kv_gather.csv`) | Shuffled vs sequential block table: ratio 1.00 in every cell — page placement costs nothing on v6e. The legacy paged kernel is 3.9–7.3× slower than XLA's dense attention over the same KV (dense: 30 µs at 512×8, 203 µs at 2048×32, 1004 µs at 8192×32), and dense XLA runs at RPA speed (2048×32: 203 µs vs 166 µs in-model). Against the dense cost the sim is −28% (512×8), +5% (2048×32), +46% (8192×8), −14% (8192×32): its decode attention is within 1.5× everywhere and the K1-fitted `-kv_bw_pct` (25) is void because it targets the legacy kernel. |
